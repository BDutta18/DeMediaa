...
export async function GET(request: Request) {
  try {
    ...
    const backendResponse = await fetch(`${backendUrl}/api/licenses/creator`, {
    ...
    console.error("[Creator Licenses] Error:", error)
    return Response.json({ error: "Failed to fetch creator data" }, { status: 500 })
  }
}
