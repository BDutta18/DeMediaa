...
export const uploadMetadataToPinata = async (
  ...
  try {
    ...
    if (authCandidates.length === 0) {
      const missingAuth = new Error(
        "Pinata credentials are missing. Set PINATA_JWT or PINATA_API_KEY/PINATA_API_SECRET.",
      )
      ;(missingAuth as any).code = "PINATA_AUTH_MISSING"
      throw missingAuth
    }
    ...
    const responseMessage =
      ...
      ""
    const combined =
      `${details || ""} ${responseMessage || ""} ${typeof rawData === "string" ? rawData : JSON.stringify(rawData || "")} ${error?.message || ""}`.toLowerCase()

    if (reason === "API_KEY_REVOKED") {
      const friendlyError = new Error(
        "Pinata API key has been revoked. Update PINATA_JWT in backend env.",
      )
    ...
    if (combined.includes("required scopes") || reason === "API_KEY_MISSING_PERMISSIONS") {
      try {
        ...
        const friendlyError = new Error(
          "Pinata key is missing required scopes for metadata upload.",
        )
