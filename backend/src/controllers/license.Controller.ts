...
export const createListing = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (nft.author.toLowerCase() !== userAddress.toLowerCase() && nft.owner.toLowerCase() !== userAddress.toLowerCase()) {
    return res.status(403).json({ success: false, message: "Only the creator or owner can list for licensing" });
  }

  const existing = await Listing.findOne({ tokenId: String(tokenId) });
  if (existing && existing.active) {
    return res.status(400).json({ success: false, message: "Active listing already exists for this token" });
  }
...
export const purchaseLicense = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!listingId || !licenseType) {
    return res.status(400).json({ success: false, message: "listingId and licenseType are required" });
  }
  ...
  if (listing.creator.toLowerCase() === buyerAddress.toLowerCase()) {
    return res.status(400).json({ success: false, message: "Creator cannot purchase their own license" });
  }
  ...
  if (existingLicense) {
    return res.status(400).json({ success: false, message: "You already hold an active license for this content" });
  }
...
export const deactivateListing = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (listing.creator.toLowerCase() !== userAddress.toLowerCase()) {
    return res.status(403).json({ success: false, message: "Only the creator can deactivate a listing" });
  }
