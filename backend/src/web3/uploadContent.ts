...
export const uploadContentToBlockchain = async (
  ...
  try {
    const { rpcServer, signer, sourceAccount, networkPassphrase, contractIds } =
      await getContractIntegration()
    ...
    try {
      ...
      const fallbackRpc = new rpc.Server(
    ...
    if (accountMatch?.[1]) {
      ...
      if (isTestnet && retryCount < 1) {
        try {
          ...
          if (fundRes.ok) {
            return await uploadContentToBlockchain(
              cid,
              fileHash,
              price,
              paymentToken,
