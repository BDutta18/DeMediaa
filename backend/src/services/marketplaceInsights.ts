...
export const scoreListing = (listing: Pick<
  IListing,
  "totalLicensesSold" | "totalRevenue" | "viewCount" | "downloadCount" | "averageRating" | "ratingCount" | "createdAt"
>): ListingInsight => {
  ...
  const ageBoost = Math.max(0, 30 - Math.floor((Date.now() - new Date(listing.createdAt).getTime()) / 86_400_000));
...
export const sortByPopularity = <T extends { popularityScore?: number; totalLicensesSold?: number; totalRevenue?: number }>(items: T[]): T[] => {
  return [...items].sort((left, right) => {
    const leftScore = left.popularityScore ?? (left.totalLicensesSold ?? 0) * 15 + (left.totalRevenue ?? 0) * 2;
    const rightScore = right.popularityScore ?? (right.totalLicensesSold ?? 0) * 15 + (right.totalRevenue ?? 0) * 2;
